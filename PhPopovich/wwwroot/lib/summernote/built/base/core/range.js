...
var WrappedRange = /** @class */ (function () {
    ...
    WrappedRange.prototype.normalize = function () {
        ...
        var getVisiblePoint = function (point, isLeftToRight) {
            ...
            if (!hasRightNode) {
                var prevPoint = dom_1["default"].prevPoint(point) || { node: null };
                hasRightNode = (dom_1["default"].isLeftEdgePointOf(point, block) || dom_1["default"].isVoid(prevPoint.node)) && !isLeftToRight;
            }
            var hasLeftNode = false;
            if (!hasLeftNode) {
                var nextPoint_1 = dom_1["default"].nextPoint(point) || { node: null };
                hasLeftNode = (dom_1["default"].isRightEdgePointOf(point, block) || dom_1["default"].isVoid(nextPoint_1.node)) && isLeftToRight;
            }
    ...
    WrappedRange.prototype.insertNode = function (node) {
        var rng = this;
        if (dom_1["default"].isText(node) || dom_1["default"].isInline(node)) {
     ...
     * @return {WrappedRange}
     */
    WrappedRange.prototype.getWordsRange = function (findAfter) {
        var endPoint = this.getEndPoint();
        var isNotTextPoint = function (point) {
            return !dom_1["default"].isCharPoint(point) && !dom_1["default"].isSpacePoint(point);
        };
        if (isNotTextPoint(endPoint)) {
            return this;
        }
        var startPoint = dom_1["default"].prevPointUntil(endPoint, isNotTextPoint);
        if (findAfter) {
            endPoint = dom_1["default"].nextPointUntil(endPoint, isNotTextPoint);
        }
        return new WrappedRange(startPoint.node, startPoint.offset, endPoint.node, endPoint.offset);
    };
    ;
     ...
     */
    WrappedRange.prototype.getWordsMatchRange = function (regex) {
        var endPoint = this.getEndPoint();
        var startPoint = dom_1["default"].prevPointUntil(endPoint, function (point) {
            if (!dom_1["default"].isCharPoint(point) && !dom_1["default"].isSpacePoint(point)) {
                return true;
            }
    ...
    create: function (sc, so, ec, eo) {
        if (arguments.length === 4) {
            ...
            if (!wrappedRange && arguments.length === 1) {
                ...
                if (dom_1["default"].isEditable(bodyElement)) {
                    bodyElement = bodyElement.lastChild;
                }
                return this.createFromBodyElement(bodyElement, dom_1["default"].emptyPara === arguments[0].innerHTML);
            }
            return wrappedRange;
        }
    },
    createFromBodyElement: function (bodyElement, isCollapseToStart) {
        if (isCollapseToStart === void 0) { isCollapseToStart = false; }
