...
public class GPTRealApi
{
    ...
    public async Task<string> Translate(string language, string text)
    {
        ...
        foreach (var oneChunk in split)
            try
            {
                ...
                var chunkText = "There is subs. Translate it to " + language + " without adding anything else and saving line breaks. If it hashtag translate it only to " + language + ":" +
                                ":\n\n" + chunkString;
    ...
    {
        var chunkText = "Generate " + count + " hashtags in " + language + " without adding anything else in one row. Hashtags should be only in " + language + ":\n\n" + title;
        var result = await _api.Chat.CreateChatCompletionAsync(chunkText);
